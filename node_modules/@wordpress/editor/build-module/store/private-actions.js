...
export const removeTemplates = items => async ({
  ...
  if (promiseResult.every(({
    ...
    if (items.length === 1) {
      ...
      __('"%s" reset.'), decodeEntities(title)) : sprintf(/* translators: %s: The template/part's name. */
      _x('"%s" deleted.', 'template part'), decodeEntities(title));
    } else {
 ...
 */
export const setDefaultRenderingMode = mode => ({
  select,
  registry
}) => {
  var _registry$select$get$;
  ...
  const renderingModes = (_registry$select$get$ = registry.select(preferencesStore).get('core', 'renderingModes')?.[theme]) !== null && _registry$select$get$ !== void 0 ? _registry$select$get$ : {};
