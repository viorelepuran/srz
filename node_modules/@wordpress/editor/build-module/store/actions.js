...
export const savePost = (options = {}) => async ({
  ...
  let error = false;
  try {
    edits = await applyFiltersAsync('editor.preSavePost', edits, options);
  } catch (err) {
    error = err;
  }
  if (!error) {
  ...
  if (!error) {
    try {
      await applyFilters('editor.__unstableSavePost', Promise.resolve(), options);
    } catch (err) {
      error = err;
    }
  }
    ...
    if (createNotice) {
      registry.dispatch(noticesStore).createInfoNotice(isDistractionFree ? __('Distraction free mode deactivated.') : __('Distraction free mode activated.'), {
        ...
        actions: [{
          ...
          onClick: () => {
            registry.batch(() => {
              registry.dispatch(preferencesStore).set('core', 'fixedToolbar', isDistractionFree);
...
export const toggleSpotlightMode = () => ({
  ...
  registry.dispatch(noticesStore).createInfoNotice(isFocusMode ? __('Spotlight mode activated.') : __('Spotlight mode deactivated.'), {
...
export const toggleTopToolbar = () => ({
  ...
  registry.dispatch(noticesStore).createInfoNotice(isTopToolbar ? __('Top toolbar activated.') : __('Top toolbar deactivated.'), {
