...
const BLOCK_REMOVAL_RULES = [{
  ...
  callback(removedBlocks) {
    ...
    if (removedTemplateBlocks.length) {
      return _n('Deleting this block will stop your post or page content from displaying on this template. It is not recommended.', 'Some of the deleted blocks will stop your post or page content from displaying on this template. It is not recommended.', removedBlocks.length);
    }
  }
}, {
  // Pattern overrides.
  ...
  callback(removedBlocks) {
    ...
    if (removedBlocksWithOverrides.length) {
      return _n('The deleted block allows instance overrides. Removing it may result in content not displaying where this pattern is used. Are you sure you want to proceed?', 'Some of the deleted blocks allow instance overrides. Removing them may result in content not displaying where this pattern is used. Are you sure you want to proceed?', removedBlocks.length);
    }
  }
}];
export default function BlockRemovalWarnings() {
  const currentPostType = useSelect(select => select(editorStore).getCurrentPostType(), []);
  const removalRulesForPostType = useMemo(() => BLOCK_REMOVAL_RULES.filter(rule => rule.postTypes.includes(currentPostType)), [currentPostType]);
  ...
  return /*#__PURE__*/_jsx(BlockRemovalWarningModal, {
